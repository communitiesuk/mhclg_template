  ...
  describe "#asset_path" do
    context "if file is stylesheet" do
      ...
      it "should return the file" do
        expect(subject.asset_path(asset_file)).to eql("#{asset_file}?#{HackneyTemplate::VERSION}")
      end
    end
    context "if not stylesheet" do
      context "if css file path passed in" do
        ...
        it "should return the correct path for a stylesheet" do
          expect(subject.asset_path(css_asset_file)).to eql("<%= HackneyTemplateAssetPath %>stylesheets/#{css_asset_file}?#{HackneyTemplate::VERSION}")
        end
      end
      context "if javascript file path passed in" do
        ...
        it "should return the correct path for a javascript file" do
          expect(subject.asset_path(js_asset_file)).to eql("<%= HackneyTemplateAssetPath %>javascripts/#{js_asset_file}?#{HackneyTemplate::VERSION}")
        end
      end
      context "if other file path passed in" do
        let(:other_asset_file) {"a/file.png"}
        it "should return the correct path for an image" do
          expect(subject.asset_path(other_asset_file)).to eql("<%= HackneyTemplateAssetPath %>images/#{other_asset_file}?#{HackneyTemplate::VERSION}")
        end
      end
    end
  end

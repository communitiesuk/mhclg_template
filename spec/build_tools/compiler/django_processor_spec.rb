require 'spec_helper'
require File.join(PROJECT_ROOT, 'build_tools/compiler/django_processor.rb')
...
describe Compiler::DjangoProcessor do
    ...
    context "if not stylesheet" do
      context "if css file path passed in" do
        ...
        it "should return the correct path for a stylesheet" do
          expect(subject.asset_path(css_asset_file)).to eql("{% static 'stylesheets/#{css_asset_file}' %}")
        end
      end
      context "if javascript file path passed in" do
        ...
        it "should return the correct path for a javascript file" do
          expect(subject.asset_path(js_asset_file)).to eql("{% static 'javascripts/#{js_asset_file}' %}")
        end
      end
      context "if other file path passed in" do
        let(:other_asset_file) {"a/file.png"}
        it "should return the correct path for an image" do
          expect(subject.asset_path(other_asset_file)).to eql("{% static 'images/#{other_asset_file}' %}")
        end
      end
    end
  end

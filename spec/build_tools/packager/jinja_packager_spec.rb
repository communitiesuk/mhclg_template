...
describe Packager::JinjaPackager do
  let(:generated_directory_path) {File.join(PROJECT_ROOT, "pkg/jinja_hackney_template-#{GovukTemplate::VERSION}")}
  ...
  context "functional" do
    describe "build" do
      ...
      it "should output the correct template" do
        ...
        expect(generated_template).to match(%r[href="{{ asset_path }}stylesheets/hackney-template\.css\?#{Regexp.escape(GovukTemplate::VERSION)}"])
      end

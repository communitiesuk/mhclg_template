...
module Publisher
  class EJSPublisher
    include Helpers
    GIT_REPO = "github.com/unboxed/hackney_template_ejs.git"
    GIT_URL = "https://#{ENV['GITHUB_TOKEN']}@#{GIT_REPO}"

    def initialize(version = HackneyTemplate::VERSION)
      @version = version
      @repo_root = Pathname.new(File.expand_path('../../..', __FILE__))
      @source_dir = @repo_root.join('pkg', "ejs_hackney_template-#{@version}")
    end

    def publish
      Dir.mktmpdir("hackney_template_ejs") do |dir|
        ...
        Dir.chdir(dir) do
          ...
          run "git add -A ."
          run "git commit -q -m 'Publishing Hackney Council Embedded JavaScript template version #{@version}'"

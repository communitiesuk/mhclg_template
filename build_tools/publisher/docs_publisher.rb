...
module Publisher
  class DocsPublisher
    ...
    def initialize(version = GovukTemplate::VERSION)
      ...
      @mustache_dir = @pkg_dir.join("mustache_hackney_template-#{@version}")
    end

    def publish
      Dir.mktmpdir("hackney_template_docs") do |dir|
        ...
        Dir.chdir(dir) do
          ...
          mustache_template = File.read("#{@mustache_dir}/views/layouts/hackney_template.html")
          ...
          run "git add -A ."
          run "git commit -q -m 'Publishing hackney_template examples version #{@version}'"

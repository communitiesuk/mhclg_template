...
module Publisher
  class PlayPublisher
    include Helpers
    GIT_REPO = "github.com/unboxed/hackney_template_play.git"
    GIT_URL = "https://#{ENV['GITHUB_TOKEN']}@#{GIT_REPO}"

    def initialize(version = HackneyTemplate::VERSION)
      @version = version
      @repo_root = Pathname.new(File.expand_path('../../..', __FILE__))
      @source_dir = @repo_root.join('pkg', "play_hackney_template-#{@version}")
    end

    def publish
      Dir.mktmpdir("hackney_template_play") do |dir|

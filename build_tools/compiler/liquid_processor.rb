...
module Compiler
  class LiquidProcessor < TemplateProcessor
    ...
    @@yield_hash = {
      ...
      homepage_url:         "{% if page.homepage_url %}{{ page.homepage_url }}{% else %}https://www.hackney.gov.uk/{% endif %}",
    ...
    def asset_path(file, options={})
      ...
      when '.css'
        "{{ site.hackney_template_assets }}/stylesheets/#{file}"
      when '.js'
        "{{ site.hackney_template_assets }}/javascripts/#{file}"
      else
        "{{ site.hackney_template_assets }}/images/#{file}"
      end
    end
  end
end
